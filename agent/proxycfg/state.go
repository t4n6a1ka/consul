	...
	for _, u := range s.proxyCfg.Upstreams {
		...
		case "": // Treat unset as the default Service type
			...
			if shouldUseDiscoveryChain {
				// Due to the current opt-outs above, the following should not happen.
				if u.DestinationName != u.Identifier() {
					panic("expected upstream name of '" + u.DestinationName + "' but got '" + u.Identifier() + "'")
				}
...
func (s *state) run() {
	...
	switch s.kind {
	case structs.ServiceKindConnectProxy:
		snap.DiscoveryChain = make(map[string]*structs.CompiledDiscoveryChain)
		snap.WatchedUpstreams = make(map[string]map[structs.DiscoveryTarget]context.CancelFunc)
		snap.WatchedUpstreamEndpoints = make(map[string]map[structs.DiscoveryTarget]structs.CheckServiceNodes)
		snap.UpstreamEndpoints = make(map[string]structs.CheckServiceNodes) // TODO(rb): deprecated
	}
...
func (s *state) handleUpdateConnectProxy(u cache.UpdateEvent, snap *ConfigSnapshot) error {
	...
	case strings.HasPrefix(u.CorrelationID, "upstream-target:"):
		...
		snap.WatchedUpstreamEndpoints[svc][target] = resp.Nodes
...
func (s *state) resetWatchesFromChain(
	...
	if _, ok := snap.WatchedUpstreams[id]; !ok {
		snap.WatchedUpstreams[id] = make(map[structs.DiscoveryTarget]context.CancelFunc)
	}
	if _, ok := snap.WatchedUpstreamEndpoints[id]; !ok {
		// TODO(rb): does this belong here?
		snap.WatchedUpstreamEndpoints[id] = make(map[structs.DiscoveryTarget]structs.CheckServiceNodes)
	}
	...
	for target, cancelFn := range snap.WatchedUpstreams[id] {
		s.logger.Printf("[DEBUG] proxycfg: discovery chain %q updated for %q; stopping watch of target %s", chain.ServiceName, id, target)
		delete(snap.WatchedUpstreams[id], target)
		delete(snap.WatchedUpstreamEndpoints[id], target) // TODO(rb): safe?
		cancelFn()
	}
	...
	for target, resolver := range targets {
		...
		s.logger.Printf("[DEBUG] proxycfg: discovery chain %q updated for %q; initializing watch of target %s", chain.ServiceName, id, target)
		...
		err = s.cache.Notify(ctx, cachetype.HealthServicesName, &structs.ServiceSpecificRequest{
			...
			// the default and makes metrics and other things much cleaner. It's
			// simpler for us if we have the type to make things unambiguous.
		}, "upstream-target:"+string(encodedTarget)+":"+id, s.ch)

		if err != nil {
			return err
		}

		snap.WatchedUpstreams[id][target] = cancel
	}
